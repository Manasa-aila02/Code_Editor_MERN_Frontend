...
export const Playground = () => {
  ...
  const { folders, loading, savePlayground } = useContext(PlaygroundContext);
  ...
  useEffect(() => {
    ...
    localStorage.removeItem('currentPlaygroundId');
  }, []);

  useEffect(() => {
    const folder = folders.find(f => f._id === folderId);
    const playground = folder?.playgrounds?.find(p => p._id === playgroundId);
  ...
  const getOutput = async (token) => {
    try {
      ...
      if (res.data.status_id <= 2) {
        return await getOutput(token); // Polling
      }
      return res.data;
    } catch (error) {
  ...
  const runCode = async () => {
    ...
    if (!language_id) {
      setCurrentOutput('Language not supported.');
      closeModal();
      return;
    }
    ...
    try {
      ...
      setCurrentOutput('Failed to run code. Please try again.');
    } finally {
      closeModal();
    }
  };
